using System;
using System.Collections.Generic;

namespace AccumapDataProcessor.Models
{
    public partial class ProdTWellviewWvtWvjob
    {
        public string Idwell { get; set; } = null!;
        public string Idrec { get; set; } = null!;
        public string? Chartofaccounts { get; set; }
        public string? Client { get; set; }
        public string? Complexityindex { get; set; }
        public double? Costfinalactual { get; set; }
        public string? Currencycode { get; set; }
        public double? Currencyexchangerate { get; set; }
        public DateTime? Dttmend { get; set; }
        public DateTime? Dttmspud { get; set; }
        public DateTime? Dttmstart { get; set; }
        public DateTime? Dttmstartplan { get; set; }
        public string? Idrectub { get; set; }
        public string? Idrectubtk { get; set; }
        public string? Idrecwellbore { get; set; }
        public string? Idrecwellboretk { get; set; }
        public string? Jobida { get; set; }
        public string? Jobidb { get; set; }
        public string? Jobidc { get; set; }
        public string? Jobsubtyp { get; set; }
        public string? Jobtyp { get; set; }
        public string? Objective { get; set; }
        public string? Objectivegeo { get; set; }
        public double? Ratetargetcond { get; set; }
        public double? Rateactualcond { get; set; }
        public double? Ratetargetgas { get; set; }
        public double? Rateactualgas { get; set; }
        public double? Ratetargetoil { get; set; }
        public double? Rateactualoil { get; set; }
        public double? Ratetargetwater { get; set; }
        public double? Rateactualwater { get; set; }
        public string? Responsiblegrp1 { get; set; }
        public string? Responsiblegrp2 { get; set; }
        public string? Responsiblegrp3 { get; set; }
        public string? Resulttechnical { get; set; }
        public string? Status1 { get; set; }
        public string? Status2 { get; set; }
        public string? Summary { get; set; }
        public string? Summarygeo { get; set; }
        public double? Targetdepth { get; set; }
        public string? Targetform { get; set; }
        public short? Userboolean1 { get; set; }
        public short? Userboolean2 { get; set; }
        public double? Usernum1 { get; set; }
        public double? Usernum2 { get; set; }
        public double? Usernum3 { get; set; }
        public double? Usernum4 { get; set; }
        public double? Usernum5 { get; set; }
        public string? Usertxt1 { get; set; }
        public string? Usertxt2 { get; set; }
        public string? Usertxt3 { get; set; }
        public string? Usertxt4 { get; set; }
        public string? Usertxt5 { get; set; }
        public string? Wvtyp { get; set; }
        public short? Syslockmeui { get; set; }
        public short? Syslockchildrenui { get; set; }
        public short? Syslockme { get; set; }
        public short? Syslockchildren { get; set; }
        public DateTime? Syslockdate { get; set; }
        public DateTime? Sysmoddate { get; set; }
        public string? Sysmoduser { get; set; }
        public DateTime? Syscreatedate { get; set; }
        public string? Syscreateuser { get; set; }
        public string? Systag { get; set; }
    }
}
